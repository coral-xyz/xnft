...
export async function findXNFTMintPDA(publisher: PublicKey, name: string): Promise<PublicKey> {
  // Mint PDA Address
  const [mintPdaAddress, _mintPdaBumpSeed] = await PublicKey.findProgramAddress(
    [Buffer.from('mint'), new PublicKey(publisher).toBuffer(), Buffer.from(name)],
    new PublicKey(programID)
  );

  // Master Edition PDA
  const [masterEditionPdaAddress, _masterEditionPdaBumpSeed] = await PublicKey.findProgramAddress(
    [
  ...
  const [xnftPdaAddress, _xnftPdaBumpSeed] = await PublicKey.findProgramAddress(
