...
export const XNFT_PROGRAM_ID = new PublicKey('BaHSGaf883GA3u8qSC5wNigcXyaScJLSBJZbALWvPcjs');
export const XNFT_KIND_OPTIONS = IDL.types[3].type.variants.map(v => v.name);
export const XNFT_L1_OPTIONS = IDL.types[4].type.variants.map(v => v.name);
export const XNFT_TAG_OPTIONS = IDL.types[5].type.variants.map(v => v.name);
